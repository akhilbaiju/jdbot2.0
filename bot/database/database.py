...
class Database(metaclass=Singleton):
    ...
    async def akdel(self, link: str):
        """
        A Funtion to delete file
        """
        await self.fcol.delete_one({"file_link": link})
        return 
    
    async def rename(self, link: str, newname: str):
        """
        A Funtion to rename file
        """
        await self.fcol.update_one({"file_link": link}, {"$set":{"file_name": newname}})
        return     
    
    
    """ async def akdel(self, target_chat: str):
        await self.fcol.delete_one({"file_link": target_chat})
        return True"""
