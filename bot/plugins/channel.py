...
async def connect(bot: Bot, update):
    ...
    if not user_id in VERIFY.get(str(chat_id)):
        await bot.send_message(
                chat_id = update.chat.id,
                text=f"""വിളച്ചിൽ എടുക്കരുത് കേട്ടോ 😒...""",
                parse_mode="html",
...
@Client.on_message(filters.command(['remove']) & (filters.user([866263993,411872315])) & filters.private, group=1)
async def rem(bot: Bot, update):
    """
    A Funtion To remove file name from jd
    """
    try:
        target_chat = update.text.split(None, 1)
        link = target_chat[1]
        await db.akdel(link)
        await update.reply_text(f"File deleted boss")
    except Exception as e:
        print(e)
       
    
@Client.on_message(filters.command(['rename']) & (filters.user([866263993,411872315])) & filters.private, group=1)
async def rename(bot: Bot, update):
    """
    A Funtion To rename files
    """
    try:        
        target_chat = update.text.split(None, 2)
        newname = target_chat[2]
        link = update.text.split()[1]
    
        await db.rename(link, newname)
        await update.reply_text(f"File Renamed From  My DB Boss \nLink: {link} \nNewname {newname}")
    except Exception as e:
        print(e)
