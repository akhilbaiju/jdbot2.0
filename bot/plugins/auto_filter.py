...
import logging
import asyncio
from bot.plugins.custfilter import manual_filters
...
async def auto_filter(bot, update):
    ...
    #group_id = update.chat.id  #-1001494898752
            ...
            if pm_file_chat: 
                ...
                if not FIND.get("bot_details"):
                    try:
                        ...
                        asyncio.sleep(e.x) # change to this if error : await asyncio.sleep(e.value)
                        bot_= await bot.get_me()
                        FIND["bot_details"] = bot_
...
async def give_filter(client, message):
    ...
    l = await auto_filter(client, message)
    if k == False and l == False:
        send_mes=await client.send_message(
